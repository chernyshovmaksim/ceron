...
class Editor_Editor {
  constructor(context) {
    ...
    this.createColor = this.wrapCommand(color => {
      
      let rng = this.getLastRange();

      const text = rng.toString();

      const additionalTextLength = text.length - rng.toString().length;

      if (additionalTextLength > 0 && this.isLimited(additionalTextLength)) {
        return;
      }

      const isTextChanged = rng.toString() !== text; // handle spaced urls from input

      let anchors = [];

      if (isTextChanged) {
        rng = rng.deleteContents();
        const anchor = rng.insertNode(external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default()('<SPAN>' + text + '</SPAN>')[0]);
        anchors.push(anchor);
      } else {
        anchors = this.style.styleNodes(rng, {
          nodeName: 'SPAN',
      ...
      external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default.a.each(anchors, (idx, anchor) => {
        external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default()(anchor).attr('style', 'color: '+color);
      });
...
class LinkDialog_LinkDialog {
  ...
  initialize() {
    ...
    const body = ['<div class="form-group note-form-group">', `<label for="note-dialog-link-txt-${this.options.id}" class="note-form-label">${this.lang.link.textToDisplay}</label>`, `<div class="form-input form-input-full"><input id="note-dialog-link-txt-${this.options.id}" class="note-link-text" type="text"/></div>`, '</div>', '<div class="form-group note-form-group">', `<label for="note-dialog-link-url-${this.options.id}" class="note-form-label">${this.lang.link.url}</label>`, `<div class="form-input form-input-full"><input id="note-dialog-link-url-${this.options.id}" class="note-link-url" type="text" value="http://"/></div>`, '</div>', !this.options.disableLinkTarget ? external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default()('<div/>').append(this.ui.checkbox({
    ...
    const buttonClass = 'form-btn note-link-btn';
...
class ImageDialog_ImageDialog {
  ...
  initialize() {
    ...
    const $container = this.options.dialogsInBody ? this.$body : this.options.container;
    const body = ['<div class="form-group note-form-group note-group-select-from-files">', '<label for="note-dialog-image-file-' + this.options.id + '" class="note-form-label">' + this.lang.image.selectFromFiles + '</label>', '<input id="note-dialog-image-file-' + this.options.id + '" class="note-image-input form-control-file note-form-control note-input" ', ' type="file" name="files" accept="image/*" multiple="multiple"/>', imageLimitation, '</div>', '<div class="form-group note-group-image-url">', '<label for="note-dialog-image-url-' + this.options.id + '" class="note-form-label">' + this.lang.image.url + '</label>', '<div class="form-input form-input-full"><input id="note-dialog-image-url-' + this.options.id + '" class="note-image-url" type="text"/></div>', '</div>'].join('');
    const buttonClass = 'form-btn note-image-btn';
...
window.adbla = external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default;
